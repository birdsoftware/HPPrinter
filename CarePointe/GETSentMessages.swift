...
class GETSentMessages {
    ...
    func getSentMessages(token: String, userID: String, dispachInstance: DispatchGroup){
        
        var sent = Array<Dictionary<String,String>>()
        ...
        let nsurlAlerts = Constants.Message.fromSent + userID
        ...
        let request = NSMutableURLRequest(url: NSURL(string: nsurlAlerts)! as URL,
                                          ...
                                          timeoutInterval: 10.0)//"http://carepointe.cloud:4300/api/inbox/sent/userId/"
        ...
        let dataTask = session.dataTask(with: request as URLRequest,
            completionHandler: { (data, response, error) -> Void in
                if (error != nil) {
                    print("GET user sent list Error:\n\(String(describing: error))")
                    ...
                    do {
                        if let data = data,  //go from a Data? type (optional Data) to a non-optional Data
                            ...
                            if(vJSON.isEmpty == false){
                                for dict in vJSON {
                                    ...
                                    sent.append(["SendTo":s, "Subject":Subject, "message":str,
                                                  "CreatedDate":date, "CreatedTime":time,"IsRead":IsRead,"ID":id])
                                }
                                
                                UserDefaults.standard.set(sent, forKey: "RESTUserSent")
                                UserDefaults.standard.synchronize()
                                print("finished GET user sent list")
                                dispachInstance.leave() // API Responded
                            }
                        ...
                        print("Error deserializing user sent list JSON: \(error)")
                        dispachInstance.leave() // API Responded
                    }
